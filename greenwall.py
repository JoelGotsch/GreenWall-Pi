...
class device:
    def __init__(self, name, gpiopin, blynk_vpin, min_pause, logger, *args, **kwargs):
        ...
        try:
            ...
            logger.error("A Gpio-Pin is assigned to at least two devices. Check gpio_settings.json")
        ...
        try:
            ...
            last_turned_on = dict()
            last_turned_on[name] = str(datetime.datetime.today()+datetime.timedelta(days=-1))
        if name not in last_turned_on.keys():
            last_turned_on[name] = str(datetime.datetime.today()+datetime.timedelta(days=-1))
            with open('last_turned_on.json', 'w') as outfile:
                json.dump(last_turned_on, outfile)
        self.last_date_turned_on = datetime.datetime.strptime(last_turned_on[name], "%Y-%m-%d %H:%M:%S.%f")
    ...
    def turn_on(self):
        if datetime.datetime.today <= self.last_date_turned_on + datetime.timedelta(seconds=self.min_pause):
            self.logger.info("Tried to turn on device '" + self.name + "' before min_pause= " + str(self.min_pause) + " seconds since the last turn on event.")
            return(0)
        self.gpio_obj.on()
    ...
    def get_target_value(self):
        ...
        for ac in filtered_plans:
            filtered_plans[ac]["last_time"] = calc_last_runtime(wdays=filtered_plans[ac]["weekdays"], start_time=filtered_plans[ac]["start_time"])
            if filtered_plans[ac]["last_time"]>last_plan["last_time"]:
                last_plan=filtered_plans[ac]

        if len(filtered_plans)==0:
            self.logger.debug("All actions filtered in function get_target_value for device " + self.name)
            return(None)
        #(3)
        ...
        for action in last_plan["actions"].keys():
            if "duration" in last_plan["actions"][action].keys():
                last_time_new = last_time + datetime.timedelta(seconds=last_plan["actions"][action]["duration"])
            ...
            if last_time_new > datetime.datetime.today():
                last_action_finished = last_action_finished and (self.name not in last_plan["actions"][action]["devices"])
                if last_action_task == "":
                    ...
                    last_action_task = [last_plan[action]["task"] for action in last_plan["actions"]][0]
                else:
                    break
            else:
                last_time=last_time_new
                last_action_finished = last_action_finished or (self.name in last_plan["actions"][action]["devices"])
        ...
        if last_action_task=="turn_on":
            ...
            self.logger.exception("While checking the target value, task '" + last_action_task + "' for device "+ self.name + " was tried, which isn't defined.")
            return(None)
...
class greenwall:
    ...
    def __init__(self, name, platform, logger=None, *args, **kwargs):
        ...
        for k in gpio_settings.keys():
            ...
            self.devices.append(device(name=curr_device["name"], gpiopin=curr_device["gpio_pin"], blynk_vpin=curr_device["vpin"], min_pause=curr_device["min_pause"], logger=self.logger))
        ...
        for dev in self.devices:
            ...
            if  tv is not None and tv != dev.get_value:
                self.logger.info("Device '" + dev.name + "' was in the wrong state and value is now set to " + str(tv))
                if tv == 1
                    dev.turn_on()
                else:
                    dev.turn_off()

        self.logger.info("Initiated green wall named '" + name + "' on " + platform)
...
def calc_last_runtime(wdays, start_time):
    ...
    possible_times = [prev_date_wday(wday)+datetime.timedelta(hours=float(hours), minutes=float(minutes)) for wday in wdays]
